...
EXTERN int OpenSeesExit(ClientData clientData, Tcl_Interp *interp, int argc,
		TCL_Char **argv);
...
int (*tclDummyLinkVarPtr)(Tcl_Interp *interp, const char *a,
		char *b, int c) = Tcl_LinkVar;
#else
int (*tclDummyLinkVarPtr)(Tcl_Interp *interp, char *a, char *b,
		int c) = Tcl_LinkVar;
#endif
...
extern "C" int EvalFileWithParameters(Tcl_Interp *interp,
		char *tclStartupFileScript, void *theInputParameters, int currentParam,
		int rank, int np);
...
void g3TclMain(int argc, char **argv, Tcl_AppInitProc * appInitProc, int rank,
		int np) {
	Tcl_Obj *resultPtr;
	...
	if (tclStartupScriptFileName != NULL) {
		...
		if (numParam == 0)
			...
			code = EvalFileWithParameters(interp, tclStartupScriptFileName, 0,
					0, 0, 1);
		...
		if (code != TCL_OK) {
			...
			if (errChannel) {
				...
				Tcl_WriteObj(errChannel,
						Tcl_GetVar2Ex(interp, "errorInfo", NULL,
								TCL_GLOBAL_ONLY));
				Tcl_WriteChars(errChannel, "\n", 1);
			}
			exitCode = 1;
