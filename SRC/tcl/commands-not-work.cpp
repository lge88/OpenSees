...
extern int OPS_ResetInput(ClientData clientData, Tcl_Interp *interp, int cArg,
		int mArg, TCL_Char **argv, Domain *domain, TclModelBuilder *builder);
...
static VariableTimeStepDirectIntegrationAnalysis *theVariableTimeStepTransientAnalysis =
		0;
...
int
opsBarrier(ClientData clientData, Tcl_Interp *interp, int argc,
		TCL_Char **argv);
...
int
neesUpload(ClientData clientData, Tcl_Interp *interp, int argc,
		TCL_Char **argv);
...
int
neesMetaData(ClientData clientData, Tcl_Interp *interp, int argc,
		TCL_Char **argv);
...
int Tcl_InterpOpenSeesObjCmd(ClientData clientData, Tcl_Interp *interp,
		int objc, Tcl_Obj *CONST objv[]) {
	int index;
	static TCL_Char *options[] = { "alias", "aliases", "create", "delete",
			"eval", "exists", "expose", "hide", "hidden", "issafe",
...
int OpenSeesAppInit(Tcl_Interp *interp) {
	...
	Tcl_CreateCommand(interp, "metaData", &neesMetaData, (ClientData) NULL,
			NULL);
	Tcl_CreateCommand(interp, "defaultUnits", &defaultUnits, (ClientData) NULL,
			NULL);
	Tcl_CreateCommand(interp, "neesUpload", &neesUpload, (ClientData) NULL,
			NULL);
...
int OPS_SetObjCmd(ClientData clientData, Tcl_Interp *interp, int objc,
		Tcl_Obj * const objv[]) {

	if (objc > 2)
		simulationInfo.addParameter(Tcl_GetString(objv[1]),
...
int OPS_SourceCmd(ClientData dummy, /* Not used. */
...
Tcl_Obj *CONST objv[]) /* Argument objects. */
{
	...
	if (objc == 4) {
		...
		if (TCL_ERROR
				== Tcl_GetIndexFromObj(interp, objv[1], options, "option",
						TCL_EXACT, &index)) {
			return TCL_ERROR;
		}
	...
	if (theStaticAnalysis != 0) {
		...
		if (argc == 6) {
			...
			if (theVariableTimeStepTransientAnalysis != 0)
				result = theVariableTimeStepTransientAnalysis->analyze(numIncr,
						dT, dtMin, dtMax, Jd);
			else {
				opserr
						<< "WARNING analyze - no variable time step transient analysis object constructed\n";
				return TCL_ERROR;
			}
...
int printModel(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	while (done == false) {
		...
		// if 'print node i j k ..' print out some nodes
			...
			currentArg++;
			res = printNode(clientData, interp, argc - currentArg,
					argv + currentArg, *output);
			done = true;
		}
...
int printNode(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if ((strcmp(argv[0], "flag") == 0) || (strcmp(argv[0], "-flag") == 0)) {
		...
		if (argc <= nodeArg) {
			opserr
					<< "WARNING print <filename> node <flag int> no int specified \n";
			return TCL_ERROR;
		}
	...
	if (nodeArg == argc) {
		...
		for (int i = 0; i < numNodes; i++) {
			...
			if (Tcl_GetInt(interp, argv[nodeArg], &nodeTag) != TCL_OK) {
				opserr << "WARNING print node failed to get integer: "
						<< argv[nodeArg] << endln;
				return TCL_ERROR;
			}
			(*theNodes)(i) = nodeTag;
...
int printElement(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if ((strcmp(argv[0], "flag") == 0) || (strcmp(argv[0], "-flag")) == 0) { // get the specified flag
		if (argc < 2) {
			opserr
					<< "WARNING print <filename> ele <flag int> no int specified \n";
			return TCL_ERROR;
		}
		if (Tcl_GetInt(interp, argv[1], &flag) != TCL_OK) {
...
int specifyAnalysis(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 2) {
		opserr
				<< "WARNING need to specify an analysis type (Static, Transient)\n";
		return TCL_ERROR;
	}
	...
	if (strcmp(argv[1], "Static") == 0) {
		...
		theStaticAnalysis = new StaticAnalysis(theDomain, *theHandler,
				*theNumberer, *theAnalysisModel, *theAlgorithm, *theSOE,
				*theStaticIntegrator, theTest);
...
// AddingSensitivity:END /////////////////////////////////
		...
		if (theHandler == 0) {
			opserr
					<< "WARNING analysis Transient dt tFinal - no ConstraintHandler\n";
		...
		if (theTransientIntegrator == 0) {
			opserr
					<< "WARNING analysis Transient dt tFinal - no Integrator specified, \n";
		...
		theTransientAnalysis = new DirectIntegrationAnalysis(theDomain,
				*theHandler, *theNumberer, *theAnalysisModel, *theAlgorithm,
...
// AddingSensitivity:END /////////////////////////////////
		...
		theVariableTimeStepTransientAnalysis =
				new VariableTimeStepDirectIntegrationAnalysis(theDomain,
						*theHandler, *theNumberer, *theAnalysisModel,
...
int specifySOE(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	// SPARSE GENERAL SOE * SOLVER
		...
		theSolver = new SuperLU(permSpec, drop_tol, panelSize, relax,
				symmetric);
...
//  }
		...
		while (solverCommands != NULL && found == false) {
			...
			if (strcmp(argv[1], solverCommands->funcName) == 0) {
				...
				OPS_ResetInput(clientData, interp, 2, argc, argv, &theDomain,
						NULL);
		...
		if (found == false) {
			...
			if (res == 0) {
				...
				ExternalClassFunction *theSolverCommand =
						new ExternalClassFunction;
...
int specifyConstraintHandler(ClientData clientData, Tcl_Interp *interp,
	...
	if (strcmp(argv[1], "Plain") == 0)
		...
		if (argc < 4) {
			opserr
					<< "WARNING: need to specify alpha: handler Penalty alpha \n";
			return TCL_ERROR;
		}
...
int specifyAlgorithm(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (strcmp(argv[1], "Linear") == 0) {
		...
		theNewAlgo = new AcceleratedNewton(*theTest, theAccel,
				incrementTangent);
	}
		...
		if (typeSearch == 0)
			theLineSearch = new InitialInterpolatedLineSearch(tol, maxIter,
					minEta, maxEta, pFlag);

		else if (typeSearch == 1)
			theLineSearch = new BisectionLineSearch(tol, maxIter, minEta,
					maxEta, pFlag);
		else if (typeSearch == 2)
			theLineSearch = new SecantLineSearch(tol, maxIter, minEta, maxEta,
					pFlag);
		else if (typeSearch == 3)
			theLineSearch = new RegulaFalsiLineSearch(tol, maxIter, minEta,
					maxEta, pFlag);
...
int specifyCTest(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (strcmp(argv[1], "FixedNumIter") == 0)
		...
		if (strcmp(argv[1], "NormUnbalance") == 0)
			...
			theNewTest = new CTestRelativeTotalNormDispIncr(tol, numIter,
					printIt, normType);
		else {
			opserr
					<< "WARNING No ConvergenceTest type (NormUnbalance, NormDispIncr, EnergyIncr, \n";
			opserr
					<< "RelativeNormUnbalance, RelativeNormDispIncr, RelativeEnergyIncr, \n";
...
int specifyIntegrator(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (strcmp(argv[1], "LoadControl") == 0) {
		...
		if (argc < 3) {
			opserr
					<< "WARNING incorrect # args - integrator LoadControl dlam <Jd dlamMin dlamMax>\n";
			return TCL_ERROR;
		}
		...
		theStaticIntegrator = new LoadControl(dLambda, numIter, minIncr,
				maxIncr);
	...
	/*********************************************************************************/
		...
		if (argc < 3) {
			opserr
					<< "WARNING integrator MinUnbalDispNorm lambda11 <Jd minLambda1j maxLambda1j>\n";
			return TCL_ERROR;
		}
		...
		theStaticIntegrator = new DisplacementControl(node, dof - 1, increment,
				&theDomain, numIter, minIncr, maxIncr);
#endif
		...
		if (argc < 3 || argc > 8) {
			opserr
					<< "WARNING integrator NewmarkExplicit gamma <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &gamma) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkExplicit gamma - undefined gamma\n";
			return TCL_ERROR;
		}
		...
		if (argc == 7) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkExplicit gamma alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkExplicit gamma alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkExplicit gamma alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkExplicit gamma alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 3)
			...
			theTransientIntegrator = new NewmarkExplicit(gamma, alphaM, betaK,
					betaKi, betaKc, updDomFlag);
		...
		if (argc != 5 && argc != 9) {
			opserr
					<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct <alphaM betaK betaKi betaKc>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &gamma) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct - undefined gamma\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[3], &beta) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct - undefined beta\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[4], &reduct) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct - undefined reduct\n";
			return TCL_ERROR;
		}
		if (argc == 9) {
			if (Tcl_GetDouble(interp, argv[5], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrReduct gamma beta reduct alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 5)
			theTransientIntegrator = new NewmarkHSIncrReduct(gamma, beta,
					reduct);
		else
			theTransientIntegrator = new NewmarkHSIncrReduct(gamma, beta,
					reduct, alphaM, betaK, betaKi, betaKc);
		...
		if (argc < 5 || argc > 11) {
			opserr
					<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit <alphaM betaK betaKi betaKc> <-normType T>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &gamma) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit - undefined gamma\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[3], &beta) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit - undefined beta\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[4], &limit) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit - undefined limit\n";
			return TCL_ERROR;
		}
		if (argc == 9 || argc == 11) {
			if (Tcl_GetDouble(interp, argv[argi], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
			argi++;
		}
		if (argi < argc) {
			if (strcmp(argv[argi], "-normType") == 0) {
				if (Tcl_GetInt(interp, argv[argi + 1], &normType) != TCL_OK) {
					opserr
							<< "WARNING integrator NewmarkHSIncrLimit gamma beta limit - undefined normType\n";
					return TCL_ERROR;
				}
			}
		}
		if (argc == 5 || argc == 7)
			theTransientIntegrator = new NewmarkHSIncrLimit(gamma, beta, limit,
					normType);
		else
			theTransientIntegrator = new NewmarkHSIncrLimit(gamma, beta, limit,
					normType, alphaM, betaK, betaKi, betaKc);
		...
		if (argc < 4 || argc > 10) {
			opserr
					<< "WARNING integrator NewmarkHSFixedNumIter gamma beta <alphaM betaK betaKi betaKc> <-polyOrder O>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &gamma) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSFixedNumIter gamma beta - undefined gamma\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[3], &beta) != TCL_OK) {
			opserr
					<< "WARNING integrator NewmarkHSFixedNumIter gamma beta - undefined beta\n";
			return TCL_ERROR;
		}
		if (argc == 8 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[argi], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSFixedNumIter gamma beta alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSFixedNumIter gamma beta alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSFixedNumIter gamma beta alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator NewmarkHSFixedNumIter gamma beta alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
			argi++;
		}
		if (argi < argc) {
			if (strcmp(argv[argi], "-polyOrder") == 0) {
				if (Tcl_GetInt(interp, argv[argi + 1], &polyOrder) != TCL_OK) {
					opserr
							<< "WARNING integrator NewmarkHSFixedNumIter gamma beta - undefined polyOrder\n";
					return TCL_ERROR;
				}
			}
		}
		if (argc == 4 || argc == 6)
			theTransientIntegrator = new NewmarkHSFixedNumIter(gamma, beta,
					polyOrder);
		else
			theTransientIntegrator = new NewmarkHSFixedNumIter(gamma, beta,
					polyOrder, alphaM, betaK, betaKi, betaKc);
		...
		if (argc != 3 && argc != 6 && argc != 7 && argc != 10) {
			opserr
					<< "WARNING integrator HHTGeneralized rhoInf <alphaM betaK betaKi betaKc>\n";
			opserr
					<< "     or integrator HHTGeneralized alphaI alphaF beta gamma <alphaM betaK betaKi betaKc>\n";
			return TCL_ERROR;
		}
		if (argc == 3 || argc == 7) {
			if (Tcl_GetDouble(interp, argv[2], &rhoInf) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized rhoInf - undefined rhoInf\n";
				return TCL_ERROR;
			}
		}
		if (argc == 6 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[2], &alphaI) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma - undefined alphaI\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &alphaF) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma - undefined alphaF\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma - undefined beta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma - undefined gamma\n";
				return TCL_ERROR;
			}
		}
		if (argc == 7) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized rhoInf alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized rhoInf alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized rhoInf alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 10) {
			if (Tcl_GetDouble(interp, argv[6], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[9], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralized alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 3)
			...
			theTransientIntegrator = new HHTGeneralized(alphaI, alphaF, beta,
					gamma);
		else if (argc == 7)
			theTransientIntegrator = new HHTGeneralized(rhoInf, alphaM, betaK,
					betaKi, betaKc);
		else if (argc == 10)
			theTransientIntegrator = new HHTGeneralized(alphaI, alphaF, beta,
		...
		if (argc < 3 || argc > 9) {
			opserr
					<< "WARNING integrator HHTExplicit alpha <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			opserr
					<< "     or integrator HHTExplicit alpha gamma <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &alpha) != TCL_OK) {
			opserr
					<< "WARNING integrator HHTExplicit alpha - undefined alpha\n";
			return TCL_ERROR;
		}
		for (int i = 3; i < argc; i++) {
			if (strcmp(argv[i], "-updateDomain") == 0) {
		...
		if (argc == 4 || argc == 8) {
			if (Tcl_GetDouble(interp, argv[3], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTExplicit alpha gamma - undefined gamma\n";
				return TCL_ERROR;
			}
		}
		if (argc == 7) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTExplicit alpha alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTExplicit alpha alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTExplicit alpha alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 8) {
			if (Tcl_GetDouble(interp, argv[4], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTExplicit alpha gamma alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTExplicit alpha gamma alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTExplicit alpha gamma alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 3)
			...
			theTransientIntegrator = new HHTExplicit(alpha, alphaM, betaK,
					betaKi, betaKc, updDomFlag);
		else if (argc == 8)
			theTransientIntegrator = new HHTExplicit(alpha, gamma, alphaM,
					betaK, betaKi, betaKc, updDomFlag);
		...
		if (argc < 4 || argc > 11) {
			opserr
					<< "WARNING integrator HHTGeneralizedExplicit rhoB alphaF <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			opserr
					<< "     or integrator HHTGeneralizedExplicit alphaI alphaF beta gamma <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			return TCL_ERROR;
		}
		...
		if (argc == 4 || argc == 8) {
			if (Tcl_GetDouble(interp, argv[2], &rhoB) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit rhoB alphaF - undefined rhoB\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &alphaF) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit rhoB alphaF - undefined alphaF\n";
				return TCL_ERROR;
			}
		}
		if (argc == 6 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[2], &alphaI) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma - undefined alphaI\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &alphaF) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma - undefined alphaF\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma - undefined beta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma - undefined gamma\n";
				return TCL_ERROR;
			}
		}
		if (argc == 8) {
			if (Tcl_GetDouble(interp, argv[4], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit rhoInf alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit rhoInf alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit rhoInf alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 10) {
			if (Tcl_GetDouble(interp, argv[6], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[9], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTGeneralizedExplicit alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 4)
			...
			theTransientIntegrator = new HHTGeneralizedExplicit(alphaI, alphaF,
					beta, gamma, updDomFlag);
		else if (argc == 8)
			theTransientIntegrator = new HHTGeneralizedExplicit(rhoB, alphaF,
					alphaM, betaK, betaKi, betaKc, updDomFlag);
		else if (argc == 10)
			theTransientIntegrator = new HHTGeneralizedExplicit(alphaI, alphaF,
					beta, gamma, alphaM, betaK, betaKi, betaKc, updDomFlag);
		...
		if (argc != 4 && argc != 7 && argc != 8 && argc != 11) {
			opserr
					<< "WARNING integrator HHTHSIncrReduct rhoInf reduct <alphaM betaK betaKi betaKc>\n";
			opserr
					<< "     or integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct <alphaM betaK betaKi betaKc>\n";
			return TCL_ERROR;
		}
		if (argc == 4 || argc == 8) {
			if (Tcl_GetDouble(interp, argv[2], &rhoInf) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct rhoInf reduct - undefined rhoInf\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &reduct) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct rhoInf reduct - undefined reduct\n";
				return TCL_ERROR;
			}
		}
		if (argc == 7 || argc == 11) {
			if (Tcl_GetDouble(interp, argv[2], &alphaI) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct - undefined alphaI\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &alphaF) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct - undefined alphaF\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct - undefined beta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct - undefined gamma\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &reduct) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct - undefined reduct\n";
				return TCL_ERROR;
			}
		}
		if (argc == 8) {
			if (Tcl_GetDouble(interp, argv[4], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct rhoInf reduct alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct rhoInf reduct alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct rhoInf reduct alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 11) {
			if (Tcl_GetDouble(interp, argv[7], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[9], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[10], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrReduct alphaI alphaF beta gamma reduct alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 4)
			...
			theTransientIntegrator = new HHTHSIncrReduct(alphaI, alphaF, beta,
					gamma, reduct);
		else if (argc == 8)
			theTransientIntegrator = new HHTHSIncrReduct(rhoInf, reduct, alphaM,
					betaK, betaKi, betaKc);
		else if (argc == 11)
			theTransientIntegrator = new HHTHSIncrReduct(alphaI, alphaF, beta,
					gamma, reduct, alphaM, betaK, betaKi, betaKc);
		...
		if (argc < 4 || argc > 13) {
			opserr
					<< "WARNING integrator HHTHSIncrLimit rhoInf limit <alphaM betaK betaKi betaKc> <-normType T>\n";
			opserr
					<< "     or integrator HHTHSIncrLimit alphaI alphaF beta gamma limit <alphaM betaK betaKi betaKc> <-normType T>\n";
			return TCL_ERROR;
		}
		if (argc == 4 || argc == 6 || argc == 8 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[argi], &rhoInf) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit rhoInf limit - undefined rhoInf\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &limit) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit rhoInf limit - undefined limit\n";
				return TCL_ERROR;
			}
			argi++;
			...
			if (Tcl_GetDouble(interp, argv[argi], &alphaI) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit alphaI alphaF beta gamma limit - undefined alphaI\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &alphaF) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit alphaI alphaF beta gamma limit - undefined alphaF\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit alphaI alphaF beta gamma limit - undefined beta\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit alphaI alphaF beta gamma limit - undefined gamma\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &limit) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit alphaI alphaF beta gamma limit - undefined limit\n";
				return TCL_ERROR;
			}
			argi++;
		}
		if (argc == 8 || argc == 10 || argc == 11 || argc == 13) {
			if (Tcl_GetDouble(interp, argv[argi], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit rhoInf limit alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit rhoInf limit alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSIncrLimit rhoInf limit alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKc) != TCL_OK) {
		...
		if (argi < argc) {
			if (strcmp(argv[argi], "-normType") == 0) {
				if (Tcl_GetInt(interp, argv[argi + 1], &normType) != TCL_OK) {
					opserr
							<< "WARNING integrator HHTHSIncrLimit rhoInf limit - undefined normType\n";
					return TCL_ERROR;
				}
			}
		}
		if (argc == 4 || argc == 6)
			theTransientIntegrator = new HHTHSIncrLimit(rhoInf, limit,
					normType);
		else if (argc == 7 || argc == 9)
			theTransientIntegrator = new HHTHSIncrLimit(alphaI, alphaF, beta,
					gamma, limit, normType);
			...
			theTransientIntegrator = new HHTHSIncrLimit(alphaI, alphaF, beta,
					gamma, limit, normType, alphaM, betaK, betaKi, betaKc);
		...
		if (argc < 3 || argc > 12) {
			opserr
					<< "WARNING integrator HHTHSFixedNumIter rhoInf <alphaM betaK betaKi betaKc> <-polyOrder O>\n";
			opserr
					<< "     or integrator HHTHSFixedNumIter alphaI alphaF beta gamma <alphaM betaK betaKi betaKc> <-polyOrder O>\n";
			return TCL_ERROR;
		}
		if (argc == 3 || argc == 5 || argc == 7 || argc == 9) {
			if (Tcl_GetDouble(interp, argv[argi], &rhoInf) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter rhoInf - undefined rhoInf\n";
				return TCL_ERROR;
			}
			argi++;
			...
			if (Tcl_GetDouble(interp, argv[argi], &alphaI) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter alphaI alphaF beta gamma - undefined alphaI\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &alphaF) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter alphaI alphaF beta gamma - undefined alphaF\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter alphaI alphaF beta gamma - undefined beta\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter alphaI alphaF beta gamma - undefined gamma\n";
				return TCL_ERROR;
			}
			argi++;
		}
		if (argc == 7 || argc == 9 || argc == 10 || argc == 12) {
			if (Tcl_GetDouble(interp, argv[argi], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter rhoInf alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter rhoInf alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHTHSFixedNumIter rhoInf alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKc) != TCL_OK) {
		...
		if (argi < argc) {
			if (strcmp(argv[argi], "-polyOrder") == 0) {
				if (Tcl_GetInt(interp, argv[argi + 1], &polyOrder) != TCL_OK) {
					opserr
							<< "WARNING integrator HHTHSFixedNumIter rhoInf - undefined polyOrder\n";
					return TCL_ERROR;
				}
			}
		}
		if (argc == 3 || argc == 5)
			...
			theTransientIntegrator = new HHTHSFixedNumIter(alphaI, alphaF, beta,
					gamma, polyOrder);
		else if (argc == 7 || argc == 9)
			theTransientIntegrator = new HHTHSFixedNumIter(rhoInf, polyOrder,
					alphaM, betaK, betaKi, betaKc);
			...
			theTransientIntegrator = new HHTHSFixedNumIter(alphaI, alphaF, beta,
					gamma, polyOrder, alphaM, betaK, betaKi, betaKc);
		...
		if (argc < 3 || argc > 10) {
			opserr
					<< "WARNING integrator AlphaOS alpha <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			opserr
					<< "     or integrator AlphaOS alpha beta gamma <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &alpha) != TCL_OK) {
		...
		if (argc == 5 || argc == 9) {
			if (Tcl_GetDouble(interp, argv[3], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha beta gamma - undefined beta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha beta gamma - undefined gamma\n";
				return TCL_ERROR;
			}
		}
		if (argc == 7) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 9) {
			if (Tcl_GetDouble(interp, argv[5], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha beta gamma alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha beta gamma alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOS alpha beta gamma alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 3)
			...
			theTransientIntegrator = new AlphaOS(alpha, alphaM, betaK, betaKi,
					betaKc, updDomFlag);
		else if (argc == 9)
			theTransientIntegrator = new AlphaOS(alpha, beta, gamma, alphaM,
					betaK, betaKi, betaKc, updDomFlag);
		...
		if (argc < 3 && argc > 11) {
			opserr
					<< "WARNING integrator AlphaOSGeneralized rhoInf <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			opserr
					<< "     or integrator AlphaOSGeneralized alphaI alphaF beta gamma <alphaM betaK betaKi betaKc> <-updateDomain>\n";
			return TCL_ERROR;
		}
		for (int i = 3; i < argc; i++) {
			if (strcmp(argv[i], "-updateDomain") == 0) {
		...
		if (argc == 3 || argc == 7) {
			if (Tcl_GetDouble(interp, argv[2], &rhoInf) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized rhoInf - undefined rhoInf\n";
				return TCL_ERROR;
			}
		}
		if (argc == 6 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[2], &alphaI) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized alphaI alphaF beta gamma - undefined alphaI\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &alphaF) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized alphaI alphaF beta gamma - undefined alphaF\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized alphaI alphaF beta gamma - undefined beta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized alphaI alphaF beta gamma - undefined gamma\n";
				return TCL_ERROR;
			}
		}
		if (argc == 7) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized rhoInf alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized rhoInf alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized rhoInf alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 10) {
			if (Tcl_GetDouble(interp, argv[6], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized alphaI alphaF beta gamma alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator AlphaOSGeneralized alphaI alphaF beta gamma alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[9], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 3)
			...
			theTransientIntegrator = new AlphaOSGeneralized(alphaI, alphaF,
					beta, gamma, updDomFlag);
		else if (argc == 7)
			theTransientIntegrator = new AlphaOSGeneralized(rhoInf, alphaM,
					betaK, betaKi, betaKc, updDomFlag);
		else if (argc == 10)
			theTransientIntegrator = new AlphaOSGeneralized(alphaI, alphaF,
					beta, gamma, alphaM, betaK, betaKi, betaKc, updDomFlag);
		...
		if (argc != 3 && argc != 5 && argc != 7 && argc != 9) {
			opserr
					<< "WARNING integrator Collocation theta <alphaM betaK betaKi betaKc>\n";
			opserr
					<< "     or integrator Collocation theta beta gamma <alphaM betaK betaKi betaKc>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &theta) != TCL_OK) {
			opserr
					<< "WARNING integrator Collocation theta - undefined theta\n";
			return TCL_ERROR;
		}
		if (argc == 5 || argc == 9) {
			if (Tcl_GetDouble(interp, argv[3], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta beta gamma - undefined beta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta beta gamma - undefined gamma\n";
				return TCL_ERROR;
			}
		}
		if (argc == 7) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 9) {
			if (Tcl_GetDouble(interp, argv[5], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta beta gamma alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta beta gamma alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator Collocation theta beta gamma alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 3)
			...
			theTransientIntegrator = new Collocation(theta, alphaM, betaK,
					betaKi, betaKc);
		else if (argc == 9)
			theTransientIntegrator = new Collocation(theta, beta, gamma, alphaM,
					betaK, betaKi, betaKc);
		...
		if (argc != 4 && argc != 6 && argc != 8 && argc != 10) {
			opserr
					<< "WARNING integrator CollocationHSIncrReduct theta reduct <alphaM betaK betaKi betaKc>\n";
			opserr
					<< "     or integrator CollocationHSIncrReduct theta beta gamma reduct <alphaM betaK betaKi betaKc>\n";
			return TCL_ERROR;
		}
		if (argc == 4 || argc == 8) {
			if (Tcl_GetDouble(interp, argv[2], &theta) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta reduct - undefined theta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &reduct) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta reduct - undefined reduct\n";
				return TCL_ERROR;
			}
		}
		if (argc == 6 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[2], &theta) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta beta gamma reduct - undefined theta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[3], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta beta gamma reduct - undefined beta\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta beta gamma reduct - undefined gamma\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &reduct) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta beta gamma reduct - undefined reduct\n";
				return TCL_ERROR;
			}
		}
		if (argc == 8) {
			if (Tcl_GetDouble(interp, argv[4], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta reduct alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta reduct alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta reduct alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 10) {
			if (Tcl_GetDouble(interp, argv[6], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta beta gamma reduct alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta beta gamma reduct alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[8], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrReduct theta beta gamma reduct alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[9], &betaKc) != TCL_OK) {
				opserr
		...
		if (argc == 4)
			...
			theTransientIntegrator = new CollocationHSIncrReduct(theta, beta,
					gamma, reduct);
		else if (argc == 8)
			theTransientIntegrator = new CollocationHSIncrReduct(theta, reduct,
					alphaM, betaK, betaKi, betaKc);
		...
		if (argc < 4 || argc > 12) {
			opserr
					<< "WARNING integrator CollocationHSIncrLimit theta limit <alphaM betaK betaKi betaKc> <-normType T>\n";
			opserr
					<< "     or integrator CollocationHSIncrLimit theta beta gamma limit <alphaM betaK betaKi betaKc> <-normType T>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &theta) != TCL_OK) {
			opserr
					<< "WARNING integrator CollocationHSIncrLimit theta limit - undefined theta\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[3], &limit) != TCL_OK) {
			opserr
					<< "WARNING integrator CollocationHSIncrLimit theta limit - undefined limit\n";
			return TCL_ERROR;
		}
		for (int i = argi; i < argc; i++) {
			if (strcmp(argv[i], "-normType") == 0) {
				...
				if (Tcl_GetInt(interp, argv[i + 1], &normType) != TCL_OK) {
					opserr
							<< "WARNING integrator CollocationHSIncrLimit theta limit - undefined normType\n";
					return TCL_ERROR;
				}
			}
		}
		if (argc == 6 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[argi], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrLimit theta beta gamma limit - undefined beta\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrLimit theta beta gamma limit - undefined gamma\n";
				return TCL_ERROR;
			}
			argi++;
		}
		if (argc == 8 || argc == 10) {
			if (Tcl_GetDouble(interp, argv[argi], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrLimit theta limit alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrLimit theta limit alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSIncrLimit theta limit alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKc) != TCL_OK) {
		...
		if (argc == 4)
			theTransientIntegrator = new CollocationHSIncrLimit(theta, limit,
					normType);
		else if (argc == 6)
			theTransientIntegrator = new CollocationHSIncrLimit(theta, beta,
					gamma, limit, normType);
		else if (argc == 8)
			theTransientIntegrator = new CollocationHSIncrLimit(theta, limit,
					normType, alphaM, betaK, betaKi, betaKc);
		...
		if (argc < 3 || argc > 11) {
			opserr
					<< "WARNING integrator CollocationHSFixedNumIter theta <alphaM betaK betaKi betaKc> <-polyOrder O>\n";
			opserr
					<< "     or integrator CollocationHSFixedNumIter theta beta gamma <alphaM betaK betaKi betaKc> <-polyOrder O>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &theta) != TCL_OK) {
			opserr
					<< "WARNING integrator CollocationHSFixedNumIter theta - undefined theta\n";
			return TCL_ERROR;
		}
		for (int i = argi; i < argc; i++) {
			if (strcmp(argv[i], "-polyOrder") == 0) {
				...
				if (Tcl_GetInt(interp, argv[i + 1], &polyOrder) != TCL_OK) {
					opserr
							<< "WARNING integrator CollocationHSFixedNumIter theta - undefined polyOrder\n";
					return TCL_ERROR;
				}
			}
		}
		if (argc == 5 || argc == 9) {
			if (Tcl_GetDouble(interp, argv[argi], &beta) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSFixedNumIter theta beta gamma - undefined beta\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &gamma) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSFixedNumIter theta beta gamma - undefined gamma\n";
				return TCL_ERROR;
			}
			argi++;
		}
		if (argc == 7 || argc == 9) {
			if (Tcl_GetDouble(interp, argv[argi], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSFixedNumIter theta alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSFixedNumIter theta alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator CollocationHSFixedNumIter theta alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			argi++;
			if (Tcl_GetDouble(interp, argv[argi], &betaKc) != TCL_OK) {
		...
		if (argc == 3)
			theTransientIntegrator = new CollocationHSFixedNumIter(theta,
					polyOrder);
		else if (argc == 5)
			theTransientIntegrator = new CollocationHSFixedNumIter(theta, beta,
					gamma, polyOrder);
		else if (argc == 7)
			theTransientIntegrator = new CollocationHSFixedNumIter(theta,
					polyOrder, alphaM, betaK, betaKi, betaKc);
		...
		if (argc != 4 && argc != 8) {
			opserr
					<< "WARNING integrator Newmark1 gamma beta <alphaM> <betaKcurrent> <betaKi> <betaKlastCommitted>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &gamma) != TCL_OK) {
			opserr
					<< "WARNING integrator Newmark1 gamma beta - undefined gamma\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[3], &beta) != TCL_OK) {
			opserr
					<< "WARNING integrator Newmark1 gamma beta - undefined beta\n";
			return TCL_ERROR;
		}
		...
		if (argc == 8 || argc == 7) {
			if (Tcl_GetDouble(interp, argv[4], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator Newmark1 gamma beta alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator Newmark1 gamma beta alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator Newmark1 gamma beta alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[7], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator Newmark1 gamma beta alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 4)
			...
			theTransientIntegrator = new Newmark1(gamma, beta, alphaM, betaK,
					betaKi, betaKc);
		...
		if (argc != 3 && argc != 7) {
			opserr
					<< "WARNING integrator HHT1 alpha <alphaM> <betaKcurrent> <betaKi> <betaKlastCommitted>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &alpha) != TCL_OK) {
		...
		if (argc == 7 || argc == 6) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator HHT1 alpha alphaM betaK betaKi betaKc - alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator HHT1 alpha alphaM betaK betaKi betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator HHT1 alpha alphaM betaK betaKi betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator HHT1 alpha alphaM betaK betaKi betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 3)
		...
		if (argc != 3 && argc != 7) {
			opserr
					<< "WARNING integrator WilsonTheta theta <alphaM> <betaK> <betaK0> <betaKc>\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[2], &theta) != TCL_OK) {
			opserr
					<< "WARNING integrator WilsonTheta theta - undefined theta\n";
			return TCL_ERROR;
		}
		if (argc == 7) {
			if (Tcl_GetDouble(interp, argv[3], &alphaM) != TCL_OK) {
				opserr
						<< "WARNING integrator WilsonTheta gamma beta alphaM betaK betaK0 betaKc- alphaM\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[4], &betaK) != TCL_OK) {
				opserr
						<< "WARNING integrator WilsonTheta gamma beta alphaM betaK betaK0 betaKc - betaK\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[5], &betaKi) != TCL_OK) {
				opserr
						<< "WARNING integrator WilsonTheta gamma beta alphaM betaK betaK0 betaKc - betaKi\n";
				return TCL_ERROR;
			}
			if (Tcl_GetDouble(interp, argv[6], &betaKc) != TCL_OK) {
				opserr
						<< "WARNING integrator WilsonTheta gamma beta alphaM betaK betaK0 betaKc - betaKc\n";
				return TCL_ERROR;
			}
		}
		if (argc == 3)
		...
		while (integratorCommands != NULL && found == false) {
			...
			if (strcmp(argv[2], integratorCommands->funcName) == 0) {
				...
				OPS_ResetInput(clientData, interp, 3, argc, argv, &theDomain,
						NULL);
...
int eigenAnalysis(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	while (loc < (argc - 1)) {
		...
		else if ((strcmp(argv[loc], "SparseGeneral") == 0)
				|| (strcmp(argv[loc], "-SparseGeneral") == 0)
				|| (strcmp(argv[loc], "-SuperLU") == 0)
				|| (strcmp(argv[loc], "SuperLU") == 0))
			typeSolver = 7;

		else if ((strcmp(argv[loc], "ProfileSPD") == 0)
				|| (strcmp(argv[loc], "-ProfileSPD") == 0))
			typeSolver = 8;
	...
	if (theEigenSOE == 0) {
		...
		if (typeSolver == 0) {
			...
			theEigenSOE = new SymBandEigenSOE(*theEigenSolver,
					*theAnalysisModel);
			...
			theEigenSOE = new FullGenEigenSOE(*theEigenSolver,
					*theAnalysisModel);
			...
			ProfileSPDLinDirectSolver *theLinSolver =
					new ProfileSPDLinDirectSolver();
	...
	if ((strcmp(argv[1], "element") == 0) || (strcmp(argv[1], "ele") == 0)) {
		...
		if (argc == 3) {
			...
			if (Tcl_GetInt(interp, argv[2], &nodeTag) != TCL_OK) {
				opserr
						<< "WARNING remove loadPattern tag? failed to read node tag: "
						<< argv[2] << endln;
				return TCL_ERROR;
			}
			if (Tcl_GetInt(interp, argv[3], &dofTag) != TCL_OK) {
				opserr
						<< "WARNING remove loadPattern tag? failed to read dof tag: "
						<< argv[3] << endln;
				return TCL_ERROR;
			}
			...
			if (argc == 5) {
				if (Tcl_GetInt(interp, argv[4], &patternTag) != TCL_OK) {
					opserr
							<< "WARNING remove loadPattern tag? failed to read pattern tag: "
							<< argv[4] << endln;
					return TCL_ERROR;
				}
...
int nodeReaction(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING nodeReaction nodeTag? dof? - could not read nodeTag? \n";
		return TCL_ERROR;
	}
	...
	if (argc > 2) {
		if (Tcl_GetInt(interp, argv[2], &dof) != TCL_OK) {
			opserr
					<< "WARNING nodeReaction nodeTag? dof? - could not read dof? \n";
			return TCL_ERROR;
		}
	}
	...
	if (dof >= 0) {
		...
		if (dof >= size) {
			opserr
					<< "WARNING nodeReaction nodeTag? dof? - dofTag? too large\n";
			return TCL_ERROR;
		}
...
int nodeEigenvector(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 3) {
		opserr
				<< "WARNING want - nodeEigenVector nodeTag? eigenVector? <dof?>\n";
		return TCL_ERROR;
	}
	...
	if (eigenvector < 0 || eigenvector >= numEigen) {
		opserr
				<< "WARNING nodeEigenvector nodeTag? dof? - eigenvecor too large\n";
		return TCL_ERROR;
	}
	...
	if (dof >= 0) {
		if (dof >= size) {
			opserr
					<< "WARNING nodeEigenvector nodeTag? dof? - dofTag? too large\n";
			return TCL_ERROR;
		}
...
int nodeCoord(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING nodeCoord nodeTag? dim? - could not read nodeTag? \n";
		return TCL_ERROR;
	}
	...
	if (argc > 2) {
		if (strcmp(argv[2], "X") == 0 || strcmp(argv[2], "x") == 0
			...
			opserr
					<< "WARNING nodeCoord nodeTag? dim? - could not read dim? \n";
			return TCL_ERROR;
		}
	}
...
int nodeAccel(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING nodeAccel nodeTag? dof? - could not read nodeTag? \n";
		return TCL_ERROR;
	}
	if (argc > 2) {
		if (Tcl_GetInt(interp, argv[2], &dof) != TCL_OK) {
			opserr
					<< "WARNING nodeAccel nodeTag? dof? - could not read dof? \n";
			return TCL_ERROR;
		}
	}
...
int nodeResponse(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING nodeResponse nodeTag? dof? - could not read nodeTag? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[2], &dof) != TCL_OK) {
	...
	if (Tcl_GetInt(interp, argv[3], &responseID) != TCL_OK) {
		opserr
				<< "WARNING nodeResponse nodeTag? dof? responseID? - could not read responseID? \n";
		return TCL_ERROR;
	}

	dof--;

	const Vector *nodalResponse = theDomain.getNodeResponse(tag,
			(NodeResponseType) responseID);
...
int calculateNodalReactions(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc == 2)
		if ((strcmp(argv[1], "-incIneria") == 0)
				|| strcmp(argv[1], "-incIneria") == 0)
			incInertia = true;
...
int sensNodeDisp(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 4) {
		interp->result =
				"WARNING want - sensNodeDisp nodeTag? dof? paramTag?\n";
		return TCL_ERROR;
	}
	...
	if (Tcl_GetInt(interp, argv[2], &dof) != TCL_OK) {
		opserr
				<< "WARNING nodeDisp nodeTag? dof? paramTag?- could not read dof? ";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[3], &paramTag) != TCL_OK) {
		opserr
				<< "WARNING nodeDisp paramTag? dof? paramTag?- could not read paramTag? ";
		return TCL_ERROR;
	}
...
int sensNodeVel(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING sensNodeVel nodeTag? dof? paramTag? - could not read nodeTag? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[2], &dof) != TCL_OK) {
		opserr
				<< "WARNING sensNodeVel nodeTag? dof? paramTag? - could not read dof? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[3], &paramTag) != TCL_OK) {
		opserr
				<< "WARNING sensNodeVel nodeTag? dof? paramTag? - could not read paramTag? \n";
		return TCL_ERROR;
	}
...
int sensNodeAccel(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 4) {
		interp->result =
				"WARNING want - sensNodeAccel nodeTag? dof? paramTag?\n";
		return TCL_ERROR;
	}
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING sensNodeAccel nodeTag? dof? paramTag? - could not read nodeTag? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[2], &dof) != TCL_OK) {
		opserr
				<< "WARNING sensNodeAccel nodeTag? dof? paramTag? - could not read dof? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[3], &paramTag) != TCL_OK) {
		opserr
				<< "WARNING sendNodeAccel nodeTag? dof? paramTag? - could not read paramTag? \n";
		return TCL_ERROR;
	}
...
int sectionForce(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING sectionForce eleTag? secNum? dof? - could not read eleTag? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[2], &secNum) != TCL_OK) {
		opserr
				<< "WARNING sectionForce eleTag? secNum? dof? - could not read secNum? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[3], &dof) != TCL_OK) {
		opserr
...
int sectionDeformation(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 4) {
		interp->result =
				"WARNING want - sectionDeformation eleTag? secNum? dof? \n";
		return TCL_ERROR;
	}
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING sectionDeformation eleTag? secNum? dof? - could not read eleTag? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[2], &secNum) != TCL_OK) {
		opserr
				<< "WARNING sectionDeformation eleTag? secNum? dof? - could not read secNum? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[3], &dof) != TCL_OK) {
		opserr
				<< "WARNING sectionDeformation eleTag? secNum? dof? - could not read dof? \n";
		return TCL_ERROR;
	}
...
int sectionLocation(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (Tcl_GetInt(interp, argv[1], &tag) != TCL_OK) {
		opserr
				<< "WARNING sectionLocation eleTag? secNum? - could not read eleTag? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetInt(interp, argv[2], &secNum) != TCL_OK) {
		opserr
				<< "WARNING sectionLocation eleTag? secNum? - could not read secNum? \n";
		return TCL_ERROR;
	}
...
int InitialStateAnalysis(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 2) {
		opserr
				<< "WARNING: Incorrect number of arguments for InitialStateAnalysis command"
				<< endln;
		return TCL_ERROR;
	}
	...
	if (strcmp(argv[1], "on") == 0) {
		...
		opserr
				<< "WARNING: Incorrect arguments - want InitialStateAnalysis on, or InitialStateAnalysis off"
				<< endln;
...
int rayleighDamping(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 5) {
		opserr
				<< "WARNING rayleigh alphaM? betaK? betaK0? betaKc? - not enough arguments to command\n";
		return TCL_ERROR;
	}
	double alphaM, betaK, betaK0, betaKc;
	if (Tcl_GetDouble(interp, argv[1], &alphaM) != TCL_OK) {
		opserr
				<< "WARNING rayleigh alphaM? betaK? betaK0? betaKc? - could not read alphaM? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetDouble(interp, argv[2], &betaK) != TCL_OK) {
		opserr
				<< "WARNING rayleigh alphaM? betaK? betaK0? betaKc? - could not read betaK? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetDouble(interp, argv[3], &betaK0) != TCL_OK) {
		opserr
				<< "WARNING rayleigh alphaM? betaK? betaK0? betaKc? - could not read betaK0? \n";
		return TCL_ERROR;
	}
	if (Tcl_GetDouble(interp, argv[4], &betaKc) != TCL_OK) {
		opserr
				<< "WARNING rayleigh alphaM? betaK? betaK0? betaKc? - could not read betaKc? \n";
		return TCL_ERROR;
	}
...
int setElementRayleighDampingFactors(ClientData clientData, Tcl_Interp *interp,
	...
	if (argc < 6) {
		opserr
				<< "WARNING setElementRayleighDampingFactors eleTag? alphaM? betaK? betaK0? betaKc? - not enough arguments to command\n";
		return TCL_ERROR;
	}
	int eleTag;
	double alphaM, betaK, betaK0, betaKc;
	...
	if (Tcl_GetInt(interp, argv[1], &eleTag) != TCL_OK) {
		opserr
				<< "WARNING rayleigh alphaM? betaK? betaK0? betaKc? - could not read eleTag? \n";
		return TCL_ERROR;
	}
...
int setPrecision(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 2) {
		opserr
				<< "WARNING setPrecision precision? - no precision value supplied\n";
		return TCL_ERROR;
	}
	int precision;
	if (Tcl_GetInt(interp, argv[1], &precision) != TCL_OK) {
		opserr
				<< "WARNING setPrecision precision? - error reading precision value supplied\n";
		return TCL_ERROR;
	}
...
int getNP(ClientData clientData, Tcl_Interp *interp, int argc,
		TCL_Char **argv) {
	int np = 1;
#ifdef _PARALLEL_INTERPRETERS
	if (theMachineBroker != 0)
...
int neesMetaData(ClientData clientData, Tcl_Interp *interp, int argc,
		TCL_Char **argv) {
	if (argc < 2)
		return -1;
	...
	while (count < argc) {
		...
		} else if ((strcmp(argv[count], "-contact") == 0)
		...
		} else if ((strcmp(argv[count], "-description") == 0)
		...
		} else if ((strcmp(argv[count], "-modelType") == 0)
		...
		} else if ((strcmp(argv[count], "-analysisType") == 0)
		...
		} else if ((strcmp(argv[count], "-elementType") == 0)
		...
		} else if ((strcmp(argv[count], "-materialType") == 0)
		...
		} else if ((strcmp(argv[count], "-loadingType") == 0)
...
int defaultUnits(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	while (count < 7) {
		if ((strcmp(argv[count], "-force") == 0)
			...
			opserr << "units - unrecognized unit: " << argv[count]
					<< " want: units -Force type? -Length type? - Time type\n";
			return -1;
		}
		count += 2;
	}
	...
	if (length == 0 || force == 0 || time == 0) {
		opserr
				<< "defaultUnits - missing a unit type want: units -Force type? -Length type? - Time type\n";
		return -1;
	}
	...
	if ((strcmp(length, "in") == 0) || (strcmp(length, "inch") == 0)) {
		...
		in = 1.0;
		opserr
				<< "defaultUnits - unknown length type, valid options: in, ft, mm, cm, m\n";
		return TCL_ERROR;
	}
	...
	if ((strcmp(force, "lb") == 0) || (strcmp(force, "lbs") == 0)) {
		...
		lb = 1.0;
		opserr
				<< "defaultUnits - unknown force type, valid options: lb, kip, N, kN\n";
		return TCL_ERROR;
	}
	...
	if ((strcmp(time, "sec") == 0) || (strcmp(time, "sec") == 0)) {
		...
		sec = 1.0;
		opserr
				<< "defaultUnits - unknown time type, valid options: sec, msec\n";
		return TCL_ERROR;
	}
...
int neesUpload(ClientData clientData, Tcl_Interp *interp, int argc,
		TCL_Char **argv) {
	if (argc < 10) {
		opserr
				<< "WARNING neesUpload -user isername? -pass passwd? -proj projID? -exp expID?\n";
		return TCL_ERROR;
	}
	int projID = 0;
	int expID = 0;
...
int OpenSeesExit(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (neesCentralProjID != 0) {
		...
		simulationInfo.neesUpload(neesCentralUser, neesCentralPasswd, pid,
				expid);
		neesCentralProjID = 0;
	}
...
int stripOpenSeesXML(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 3) {
		opserr
				<< "ERROR incorrect # args - stripXML input.xml output.dat <output.xml>\n";
		return -1;
	}
	...
	if (outputDescriptiveFile != 0) {
		...
		if (theOutputDescriptiveFile.bad()) {
			opserr << "stripXML - error opening input file: "
					<< outputDescriptiveFile << endln;
			return -1;
		}
	}
...
int convertBinaryToText(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 3) {
		opserr
				<< "ERROR incorrect # args - convertBinaryToText inputFile outputFile\n";
		return -1;
	}
...
int convertTextToBinary(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (argc < 3) {
		opserr
				<< "ERROR incorrect # args - convertTextToBinary inputFile outputFile\n";
		return -1;
	}
	...
	if (argc > 1) {
		...
		while (currentArg < argc && argv[currentArg] != NULL) {
			...
			if ((strcmp(argv[currentArg], "-par") == 0)
				...
				currentArg += 2;
			} else if ((strcmp(argv[currentArg], "-upload") == 0)
					|| (strcmp(argv[currentArg], "-UPLOAD") == 0)) {
				bool more = true;
				currentArg++;
...
extern "C" int EvalFileWithParameters(Tcl_Interp *interp,
		char *tclStartupFileScript, OpenSeesTcl_Parameter *theInputParameters,
	...
	if (currentParam < numParam) {
		...
		if ((count % np) == rank) {
			...
			for (int i = 0; i < numParam; i++) {
				...
				Tcl_SetVar(interp, paramNames[i], paramValues[i],
						TCL_GLOBAL_ONLY);
...
int setParameter(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (strstr(argv[argLoc], "-ele") != 0) {
		...
		if ((strcmp(argv[argLoc], "-ele") == 0)
			...
			if (argLoc == argc) {
				opserr
						<< "WARNING: No response type specified for element recorder. "
						<< endln;
				return TCL_ERROR;
			}
			...
			if (numEle == 0) {
				while (((theEle = theEles()) != 0)) {
					int theResult = theEle->setParameter(&argv[argLoc],
							argc - argLoc, *theParameter);
					if (theResult != -1) {
						objectCount++;
...
int printModelGID(ClientData clientData, Tcl_Interp *interp, int argc,
	...
	if (hasQuad4 == 1) {
		// Print HEADER
		outputFile
				<< "MESH \"4NMESH\" dimension 3 ElemType Quadrilateral Nnode 4"
				<< endln;
